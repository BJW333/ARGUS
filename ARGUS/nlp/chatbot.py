...
class Chatbot:
    def __init__(self, model_id="llama3-8b-q4_0-argus", max_tokens=400, max_generate_attempts: int = 10):
        ...
        logging.info("[ARGUS LLM] Model ready.")
    ...
    def get_embedding(self, text): 
        """        
        Get the embedding for a given text, caching results to avoid recomputation.
        """
        if text not in self.embedding_cache:
            self.embedding_cache[text] = self.semantic_model.encode(text, convert_to_tensor=True)
    ...
    def ensure_ollama_running(self):
        if not self.is_ollama_running():
            logging.info(f"[ARGUS LLM] Ollama not running. Starting model '{self.model_name}'...")
            subprocess.Popen(["ollama", "run", self.model_name])
            self.wait_for_ollama_ready()  # Give Ollama time to initialize
        else:
            logging.info("[ARGUS LLM] Ollama is already running.")
    ...
    def calculate_confidencevalue_response(self, user_input, bot_response, candidates):
        ...
        print(f"Final confidence score after np.clip: {final_confidence}")
        
        #return the final confidence score and the individual factors
        factors = {"S": S, "D": D, "R": R} # Debugging line to see individual factors
        print(f"Factors: {factors}")  # Debugging line to see individual factors
    ...
    def generate_ARGUS_llmresponse(self, input_sentence, num_candidates=5, temperature=0.75, top_k=30, top_p=0.9): #may have lower num_canidates to 3
        attempts = 0
        ...
        while len(all_candidates) < num_candidates and attempts < self.max_generate_attempts:
            ...
            try:
                ...
                logging.error(f"[ARGUS LLM] Error during generation: {e}")
    ...
    def crossover_text_advanced(self, parent_a: str, parent_b: str) -> str:
        ...
        print("Child text uncleaned:", child_text) #debugging line
    ...
    def mutate_semantic_drift(self, base_text: str, num_variants: int = 5, sim_range=(0.75, 0.95)) -> str:
        ...
        print(f"Best semantic drift variant: {best_variant}") #debugging line
    ...
    def ga_rerank_candidates(self, user_input, candidates,
        ...
        #print("population:", population) #debugging line to see initial population
        #print("candidates:", candidates) #debugging line to see initial candidates
        ...
        print(f"[GA Final] Best candidate (confidence = {best_confidence:.2f}): {best_candidate}")
