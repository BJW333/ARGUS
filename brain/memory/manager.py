...
# Platform I/O is injected via callbacks (prompt_fn, output_fn)
# so this module stays platform-agnostic and works on desktop or robot.
...
class MemoryManager():
    ...
    IDENTITY_RE = re.compile(
        r"(?:call me|my name is|i'm|i am|i am called)\s+"
        r"([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)"
        r"(?:\s*[.!,]|$)"
    )
    ...
    def __init__(self, memory_path: str = "memory",
                 prompt_fn=None, output_fn=None) -> None:
        """
        Args:
            memory_path: Directory for personality.json, short_term.json, long_term.json
            prompt_fn:   Callable(question_str) → answer_str or None.
                         Used for interactive dialogs (e.g. name change confirmation).
                         Desktop: speak + listen.  Robot: speak + listen.  Test: mock.
                         If None, interactive features are silently skipped.
            output_fn:   Callable(message_str) → None.
                         Used to display status messages.
                         Desktop: print_to_gui.  Robot: log.  Test: print.
                         If None, messages go to print().
        """
    ...
    def forget_or_update_traits(self, user_msg: str):
        ...
        if "forget my name" in lowered or re.search(r"\b(forget|reset) (my )?name\b", lowered):
            ...
            if "name" in self.personality_memory:
                if self._prompt:
                    self._output("Do you really want to change your name in the system?")
                    confirm = self._prompt("Do you really want to change your name in the system? Answer Yes or No")
                    if confirm and "yes" in confirm.lower():
                        self._output("What do you want your name changed to?")
                        newname = self._prompt("What do you want your name changed to?")
                        if newname:
                            self.personality_memory["name"] = newname
                            self._output(f"This is the new updated name: {newname}")
                    else:
                        self._output("Ok I won't change your name in the system")
                # If no prompt_fn, skip the interactive name change silently
