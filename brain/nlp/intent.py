...
import time
import json
from config_metrics.main_config import nlp, ARGUS_DIR
...
DATASET_PATH = ARGUS_DIR / "data/training" / "intent_hlrf.jsonl"
...
class intentrecognition():   
    ...
    def unified_intent_pipeline(self, user_input):
        ...
        clauses = [user_input.strip()]

        results = []
        for clause in clauses:
            cleaned = clause.strip()
            
            #special case for workspace intent
            low = cleaned.lower()
            if re.search(r"\b(work\s*space|work-space|workspace)(s)?\b", low):
                final_predicted_intent = "workspace"
                print_to_gui(f"| Final Intent: {final_predicted_intent} |")
                #results.append((final_predicted_intent, clause))
                results.append((final_predicted_intent, clause, 1.0))
                continue
            ...
            # Single decision: agreement-based confidence.
            # A weak ML head can be confidently wrong, but two independent
            # classifiers landing on the same real intent is a much stronger
            # signal. Disagreement falls through to Claude.
            if ml_predicted_intent == rulebased_predicted_intent \
               and ml_predicted_intent != "ai_response":
                final_predicted_intent = ml_predicted_intent
                final_confidence = 1.0
            elif rulebased_predicted_intent != "ai_response" \
                 and rulebased_confidence >= 1.0 \
                 and ml_confidence < 0.5:
                final_predicted_intent = rulebased_predicted_intent
            ...
            results.append((final_predicted_intent, clause, final_confidence))
