...
class BackendBridge(QObject):
    ...
    messageChunkReceived = Signal(str)  # streaming token chunk
    ...
    def __init__(self, parent=None):
        ...
        self._bus.assistant_chunk.connect(self._on_chunk)
    ...
    def sendMessage(self, text: str) -> None:
        ...
        # GUI input is typed → silence TTS. Wake-word path stays default "voice".
    ...
    def _on_backend_output(self, text: str) -> None:
        ...
        We keep the full multi-line blob intact so markdown code blocks,
        bullet lists, etc. render correctly inside a single chat bubble.
        """
        import re
        ...
        if first.startswith("Me  -->") or first.startswith("YOU:"):
            role = "user"
            parts = line.split("-->", 1)
            if len(parts) == 2:
                line = parts[1].strip()

        elif first.startswith("ARGUS:"):
            role = "assistant"
            line = line[len("ARGUS:"):].strip()

        elif (
            first.startswith("[HLRF]")
            or first.startswith("[DEBUG]")
            or first.startswith("[print_to_gui]")
        ):
            role = "system"
    ...
    def _on_chunk(self, text: str) -> None:
        """Forward streaming chunk to QML."""
        self.messageChunkReceived.emit(text)
    #─────────────────────────────────────────────────────
