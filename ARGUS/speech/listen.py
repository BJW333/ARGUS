...
from core.input_bus import send as submit_user_input, print_to_gui #, debug_bus_id
#debug_bus_id("listen")

# Keep this sample rate in sync with AnimatedCanvas (44100 by default)
RATE = 44100

def _feed_audio_level_to_gui(audio):
    """Send RMS from SR audio to QML GUI for visualization."""
    try:
        ...
        if hasattr(app, 'backend'):
            ...
            backend.setAudioLevel(min(1.0, rms * 12.0))
    ...
    try:
        with sr.Microphone(sample_rate=RATE) as source:
            ...
            try: 
                ...
                _feed_audio_level_to_gui(voiceinputaudio)  # Feed volume to the canvas pulse
                inputbyvoice = recognizer.recognize_google(voiceinputaudio, language='en-us')
                print_to_gui("Me  --> ", inputbyvoice) #print to gui so it shows up
                return inputbyvoice
        ...
        with sr.Microphone(sample_rate=RATE) as source:
            ...
            while True:
                ...
                try:
                    ...
                    _feed_audio_level_to_gui(audio) # Feed volume to the canvas pulse
                    ...
                    if "argus" in maininput.lower():
                        ...
                        #prints to gui on the line below so it only prints with argus confirmed
                        print_to_gui("Me  --> ", maincommand) 
