"""
check_internet skill — auto-generated from TOOL_DEFINITIONS.

Wraps the existing network.check capability. Behavior is unchanged from the
...
def handle(**_) -> str:
    ...
    cap = REGISTRY.get("network.check")
    if cap is None:
        return "[error] network.check not registered"
    return cap.handler()
