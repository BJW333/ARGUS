...
def train_with_feedback(chatbot, sampled_dataset, feedback_data, epochs): #could include sampled_dataset as a variable so that the main dataset is also in the feedback data
    ...
    #train_dataset = tf.data.Dataset.from_tensor_slices((input_sequences, target_sequences)).shuffle(len(input_sequences)).batch(32) dont want shuffle because might mess things up
...
class Chatbot:
    ...
    def generate_seq2seqresponse(self, input_sentence, num_candidates=5, temperature=0.6, top_k=30):
        ...
        for _ in range(num_candidates):
            ...
            print("This is the canidates:", candidate_text)
...
def process_user_input(user_input):
    ...
    #usage of training with feedback and historical data
        ...
        confidence_threshold = max(0.4, min(0.8, 1 - normalized_reward))
        #orginal line = confidence_threshold = max(0.4, min(0.8, 1 - (reward / 30)))  above line is new 
