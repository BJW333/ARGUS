...
class intentrecognition():
    ...
    def intentunderstand(self, user_input):
        searchwikilist = ['find me information', 'what is', 'who is', 'search', 'tell me about'] #'find'
        weblist = ['google', 'web', 'look up']
        ...
        #objectrecoginit = ['what do you see']
        ...
        if any(keyword in user_input_lower for keyword in searchwikilist):
            return "searchwiki"
        
        if any(keyword in user_input_lower for keyword in weblist):
            return "websearch"
        
        if any(keyword in user_input_lower for keyword in timelist):
            return "time"
        
        if any(keyword in user_input_lower for keyword in openapp):
            return "open"
        
        if any(keyword in user_input_lower for keyword in closeapp):
            return "close"
        
        if any(keyword in user_input_lower for keyword in newsget):
            return "news"
        
        if any(keyword in user_input_lower for keyword in internetstatus):
            return "connectionwithinternet"
        
        if any(keyword in user_input_lower for keyword in timerprogram):
            return "timer"
        
        if any(keyword in user_input_lower for keyword in coinflip):
            return "coinflip"
        
        if any(keyword in user_input_lower for keyword in exitprogram):
            return "exit"
        
        #if any(keyword in user_input_lower for keyword in objectrecoginit):
        #    return "objrecog"
        
        if any(keyword in user_input_lower for keyword in stock_intents):
            return "stock_data"

        if any(keyword in user_input_lower for keyword in weather_intents):
            return "weather_data"

        if any(keyword in user_input_lower for keyword in flight_intents):
            return "flight_data"

        if any(keyword in user_input_lower for keyword in crypto_intents):
            return "crypto_data"
...
def process_user_input(user_input):
    ...
    #    objectrecognitionrun()
        ...
        stock_ticker = yf.Ticker(stock_symbol).info.get('symbol', 'Ticker not found')
