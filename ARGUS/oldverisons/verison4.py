...
openai.api_key = 'sk-proj-6ZpsOrEloKi1l2XSStYBT3BlbkFJigIIsIEIeL5OEcSLgNmy'
...
(text for text in all_texts), target_vocab_size=2**13)
    ...
    while True:
        ...
        if "Josh" in intro:
            #os.system(f"afplay {wake_word_sound_path}")
            print("Taking input")
            user_input = ""
            with sr.Microphone() as source:
                recognizer.adjust_for_ambient_noise(source)
                audiotask = recognizer.listen(source)
            try:
                user_input = recognizer.recognize_google(audiotask, language='en-us')
