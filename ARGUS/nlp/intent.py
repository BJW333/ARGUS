...
def log_intent_example(text: str, final_intent: str, meta: dict | None = None, path: str = DATASET_PATH):
    ...
    try:
        ...
        print_to_gui(f"[HLRF] Target file: {p}")
        print_to_gui(f"[HLRF] Lock file:   {lock_path}")

        with FileLock(lock_path, timeout=5):
            with p.open("a", encoding="utf-8") as f:
                f.write(json.dumps(rec, ensure_ascii=False) + "\n")

        print_to_gui(f"[HLRF] Write OK. Size now: {p.stat().st_size} bytes")
    except Exception as e:
        # Surface the error loudly so you can fix it fast
        print_to_gui(f"[HLRF][ERROR] {type(e).__name__}: {e}")
        try:
