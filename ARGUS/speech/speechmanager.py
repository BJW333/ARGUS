...
class SpeechManager:
    ...
    def _speak_text(self, text, voice="en_UK/apope_low"):
        ...
        playback = self.current_playback  #local

        #Wait here (in the SpeechManager thread) until playback finishes
        while playback.is_playing():
            if self.stop_flag.is_set():
                playback.stop()
                #Optional: also clear any queued speech when interrupted
                self.clear_queue()
                break
