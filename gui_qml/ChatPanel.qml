...
Item {
            ...
            Row {
                ...
                Rectangle {
                    ...
                    TextEdit {
                        ...
                        readOnly: true
                        selectByMouse: true
                        selectionColor: Qt.rgba(0.4, 0.7, 1.0, 0.35)
                        color: isUser
                            ? Qt.rgba(1, 1, 1, 1)
                            : (isSystem 
                                ? Qt.rgba(1, 1, 1, 0.6)
    ...
    Component.onCompleted: {
        ...
        Backend.messageChunkReceived.connect(function(chunk) {
            var idx = messagesModel.count - 1
            if (idx >= 0 && messagesModel.get(idx).role === "assistant") {
                var current = messagesModel.get(idx).text
                messagesModel.setProperty(idx, "text", current + chunk)
            }
            if (listView.stickToBottom)
                Qt.callLater(function(){ listView.positionViewAtEnd() })
        })
