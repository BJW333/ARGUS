...
class SpeechManager:
    def __init__(self, pitch_semitones=0.5, speed_factor=1.05): #orginal speed was 1.06 and orginal pitch 0.5
        ...
        self.thread = threading.Thread(target=self._speak_loop, daemon=True, name="SpeechManager")
        self.thread.start()
    ...
    #old clear queue method that directly accessed the queue's internals was not thread-safe    
    ...
    #new thread-safe clear queue method if issue revert to the old one above
    def clear_queue(self):
        while True:
            try:
    ...
    def _speak_text(self, text, voice="en_UK/apope_low"):
        ...
        # ── Route audio through AEC engine (replaces simpleaudio) ──
        # Convert to int16 mono 16kHz — what AEC engine accepts.
        # AEC engine plays through speakers AND uses as reference signal,
        # so the mic's capture of this audio gets subtracted out before
        # OWW/Whisper see it. Result: ARGUS cannot hear itself.
        audio_16k = audio.set_frame_rate(16000).set_channels(1).set_sample_width(2)
        samples = np.frombuffer(audio_16k.raw_data, dtype=np.int16)
        
        aec_engine.queue_playback(samples)
        
        # Wait for playback to drain (poll is_playing flag)
        while aec_engine.is_playing:
            if self.stop_flag.is_set():
                aec_engine.clear_playback()
                self.clear_queue()
                break
            time.sleep(0.01)
    ...
    #        # Drop any pending audio in the AEC engine playback queue
    #        aec_engine.clear_playback()
    
    def stop(self):
        # Always clear pending speech queue first, even if not actively speaking.
        # Prevents the next-in-line utterance from starting after a wake word.
        log_debug(f"[SM] stop() called (speaking={self.speaking})")
        self.clear_queue()
        if self.speaking:
            self.stop_flag.set()
            # Stop mimic3 subprocess if still running.
            # terminate() sends SIGTERM (graceful); kill() sends SIGKILL (immediate).
            # Use both so a wedged mimic3 process doesn't keep generating audio.
            if self.current_process and self.current_process.poll() is None:
                try:
                    self.current_process.terminate()
