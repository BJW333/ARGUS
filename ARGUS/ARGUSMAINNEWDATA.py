...
def process_user_input(user_input):
    ...
    if intent == "exit":
        ...
        formatted = [name.replace('_', ' ').title() for name in results]
        
        #natural language construction code
        people = [name for name in formatted if " " in name]  #crude filter for names
        objects = [obj for obj in formatted if " " not in obj]
        
        parts = []
        if people:
            if len(people) == 1:
                parts.append(f"I see {people[0]}")
            else:
                people_list = ", ".join(people[:-1]) + f", and {people[-1]}"
                parts.append(f"I saw people named {people_list}")
        if objects:
            if len(objects) == 1:
                parts.append(f"a {objects[0]}")
            else:
                object_list = ", ".join(objects[:-1]) + f", and a {objects[-1]}"
                parts.append(f"{object_list}")
        
        final_sentence = " and ".join(parts) + "."
        
        print("Final output sentence of objpersonrecog:", final_sentence)    
