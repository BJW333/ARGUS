...
class Chatbot:
    ...
    def calculate_confidencevalue_response(self, user_input, bot_response, fitness_score, candidates):
        ...
        confidence = (0.3 * F) + (0.3 * S) + (0.4 * D) #more diversty in responses
    ...
    def generate_seq2seqresponse(self, input_sentence, num_candidates=5, temperature=0.6, top_k=30,
                             repetition_penalty=1.2, no_repeat_ngram_size=3): # orginal no_repeat_ngram_size=3 is the orginal value
        ...
        vocab_size = self.vocab_size  #assumes you've stored the vocab size
        ...
        for _ in range(num_candidates):
            ...
            for _ in range(self.max_length):
                #get the raw model predictions (logits) for the current decoder input
                predictions = self.model([input_tensor, decoder_input])
                #extract logits for the last time step (shape: [vocab_size])
                logits = predictions[:, -1, :].numpy()[0]

                #Repetition Penalty
                #for any token already generated divide its logit by the penalty factor
                for token in set(response_ids):
                ...
                predicted_probabilities = tf.nn.softmax(logits, axis=-1).numpy()

                #apply No repeat Ngram constraint 
                if len(response_ids) >= no_repeat_ngram_size - 1:
                    #get the most recent n-1 tokens as the prefix
                    ngram_prefix = response_ids[-(no_repeat_ngram_size - 1):]
                    banned_tokens = []

                    #loop over the entire vocabulary (could also restrict to top-k candidates)
                    for token in range(vocab_size):
                        candidate_ngram = ngram_prefix + [token]
                        #check if this ngram already appears in response ids
                        for i in range(len(response_ids) - no_repeat_ngram_size + 1):
                            if response_ids[i:i + no_repeat_ngram_size] == candidate_ngram:
                                banned_tokens.append(token)
                                break

                    #zero out probabilities for banned tokens and renormalize if needed
                    for token in banned_tokens:
                        predicted_probabilities[token] = 0.0
                    total_prob = np.sum(predicted_probabilities)
                    if total_prob > 0:
                        predicted_probabilities /= total_prob

                #top-k algo
                top_k_indices = np.argsort(predicted_probabilities)[-top_k:]
                top_k_probs = predicted_probabilities[top_k_indices]
                top_k_probs /= np.sum(top_k_probs)  #renormalize the top-k probabilities
                ...
                #append token to response and update the decoder input
            ...
            print("This is the candidate:", candidate_text)
            all_candidates.append(candidate_text)

        # 4) Return the list of candidate responses
    ...
    def mutate_text_all_in_one(self, text: str) -> str:
        ...
        def get_similar_word(word, threshold=0.7, top_n=10):
            """
            Return a semantically similar word (if available) using spaCy's built-in word vectors.
    ...
    def ga_rerank_candidates(self, user_input, candidates,
        ...
        scores = [fitness_func(c) for c in population]
...
def process_user_input(user_input):
    ...
    #usage of training with feedback and historical data
        ...
        candidates = chatbot.generate_seq2seqresponse(
            ...
            num_candidates=5,  # Adjust the number of candidates as needed
            temperature=0.6
        )  # temp was 0.5
        ...
        """
        Our reward system ranges form -30 to 30 and maybe it be worth it to range our normalization from -1 to 1
...
(text for text in all_texts), target_vocab_size=2**13)
    ...
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
        initial_learning_rate, decay_steps=634, decay_rate=0.998, staircase=True) #orginal decay steps 100000 the decay steps value after using the equation is 634 with current amount of data
