"""
get_time skill — auto-generated from TOOL_DEFINITIONS.

Wraps the existing time.get_current capability. Behavior is unchanged from the
...
def handle(**_) -> str:
    ...
    cap = REGISTRY.get("time.get_current")
    if cap is None:
        return "[error] time.get_current not registered"
    return cap.handler()
