...
from config_metrics.main_config import script_dir, nlp, get_semantic_model, get_grammar_tool
from concurrent.futures import ThreadPoolExecutor, as_completed

# ── Backend abstraction (Phase 1) ─────────────────────────────
from brain.nlp.backends import (
    OllamaBackend, AnthropicBackend, OpenAIBackend, RouterBackend,
)

def _build_backend():
    """
    Construct the backend stack from environment variables.

    Env vars (all optional with defaults):
        ARGUS_MODE          : "auto" | "api" | "local"  (default "auto")
        ARGUS_API_PROVIDER  : "anthropic" | "openai"     (default "anthropic")
        ARGUS_API_MODEL     : model name override
        ARGUS_LOCAL_MODEL   : ollama model name          (default "argus-40b")
        ANTHROPIC_API_KEY   : required if provider=anthropic
        OPENAI_API_KEY      : required if provider=openai
    """
    mode = os.getenv("ARGUS_MODE", "auto").lower()
    provider = os.getenv("ARGUS_API_PROVIDER", "anthropic").lower()
    api_model = os.getenv("ARGUS_API_MODEL")
    local_model = os.getenv("ARGUS_LOCAL_MODEL", "argus-40b")

    if provider == "openai":
        primary = OpenAIBackend(model=api_model or "gpt-4o")
    else:
        primary = AnthropicBackend(model=api_model or "claude-opus-4-7")

    fallback = OllamaBackend(model=local_model)
    return RouterBackend(primary=primary, fallback=fallback, mode=mode)

class Chatbot:
    def __init__(self, model_id="argus-40b", max_tokens=2048, max_generate_attempts: int = 10, reward_system=None):
        self.model_name = model_id
        #self.max_tokens = max_tokens
        
        #TOKEN CONTROL (TWO KNOBS)
        self.candidate_max_tokens = max_tokens   #KNOB 1: short candidates for GA
        self.final_max_tokens = 8192   #KNOB 2: long final answer (expand) set to 4096 if you want big answers 
        ...
        self.paraphraser = pipeline(
            ...
            truncation=True,  #only do small and medium outputs 
            tokenizer=tokenizer,
            device=device
        )
        #print("Device being used for paraphrasing:", {device})    #Debugging line to see which device is used
        self.enable_grammar_correction = True   #Enable grammar correction by default

        # ── Backend stack (Phase 1) ────────────────────────
        # self.backend handles all main LLM calls. Auto-routes between
        # API (Claude/GPT) and local Ollama with refusal-based fallback.
        # The backend constructor handles Ollama startup if needed.
        self.backend = _build_backend()
        log_debug(f"[Chatbot] Backend ready. mode={self.backend.mode} "
                  f"primary={self.backend.primary.name} "
                  f"fallback={self.backend.fallback.name}")
        ...
        if self.reward_system is None:
            ...
            self.reward_system = DynamicRewardSystem()  #Fallback
    ...
    def set_mode(self, mode: str) -> None:
        """Switch backend routing mode at runtime: 'auto' | 'api' | 'local'."""
    ...
    def calculate_confidencevalue_response(self, user_input, bot_response, candidates):
        ...
        #If similarities is empty, use a neutral default (avg_similarity=0.5) so D=0.5.
        #This is a heuristic, not a measured diversity value.
    ...
    def choose_num_predict(self, prompt: str, num_ctx: int = 32768, max_out: int = 8192, buffer: int = 300):
        budget = num_ctx - self.estimate_tokens(prompt) - buffer
        # allow short outputs when prompt is long
        return max(32, min(max_out, budget))
    
    # def _generate_single_candidate(self, system_prompt, user_prompt, temperature, top_k, top_p, num_predict):
    #     """Helper function to generate one candidate (runs in thread)"""
    #     try:
    #         timeout = max(300, 120 + int(num_predict) / 3) #simple heurstic 
    ...
    #                 "top_k": int(top_k),
    #                 "top_p": float(top_p),
    #                 # Optional but helps prevent regurgitation:
    #                 "stop": ["<<SYSTEM CONTEXT", "USER_MESSAGE:", "DRAFT_ANSWER:"]
    #             }
    #         }
    ...
    #         log_debug(f"[ARGUS LLM] Error: {e}")
    #         return None
        
    def _generate_single_candidate(self, system_prompt, user_prompt, temperature, top_k, top_p, num_predict):
        """Generate one candidate. Delegates to the backend router (Phase 1)."""
    ...
    def _classify(self, user_prompt: str) -> str:
        """
        Fast classification using argus-router (3B uncensored).
        
        Uses few-shot prompting in the user message. The system prompt
        is baked into the argus-router modelfile.
        ~300ms per call, no thinking overhead.
        """
        try:
            response = requests.post(
                f"http://{self.ollama_host}:{self.ollama_port}/api/chat",
            ...
            response.raise_for_status()
            answer = (response.json().get("message", {}).get("content", "") or "").strip().lower()
            log_debug(f"[Router] answer='{answer}'")
            return answer
        except Exception as e:
            log_debug(f"[Router] Error: {e}")
            return ""
        
    # def _generate_streaming(self, system_prompt, user_prompt, temperature, top_k, top_p, num_predict, on_chunk=None):
    #     """
    #     Streaming generation — yields tokens as Ollama produces them.
    #     Calls on_chunk(token_text) for each piece so the GUI can
    #     display text as it arrives instead of waiting for the full response.
    #     Returns the complete response string when done.
    #     """
    #     try:
    #         timeout = max(600, 120 + int(num_predict) / 3)
    ...
    #     except Exception as e:
    #         log_debug(f"[ARGUS LLM] Streaming error: {e}")
    #         return None
        
    def _generate_streaming(self, system_prompt, user_prompt, temperature, top_k, top_p, num_predict, on_chunk=None):
        """
        Streaming generation. Delegates to the backend router (Phase 1).
        Calls on_chunk(token) for each token; returns the complete text.
        """
        return self.backend.stream(
            system_prompt or "",
            user_prompt or "",
            on_chunk or (lambda t: None),
            temperature=float(temperature),
            num_predict=int(num_predict),
            top_k=int(top_k),
            top_p=float(top_p),
    ...
    def generate_ARGUS_llmresponse(self, input_sentence, memory_prefix, num_candidates=5, temperature=0.75, top_k=30, top_p=0.9):
        """Modified to run candidates in parallel - GUI won't freeze!"""
        ...
        # token budgeting: estimate on combined text
        combined_for_budget = (system_prompt or "") + "\n\n" + (user_prompt or "")
        #predict the max num of tokens to use 
        num_predict = self.choose_num_predict(combined_for_budget, max_out=self.candidate_max_tokens)

        #Generate all candidates in parallel with varied temperatures
        futures = []
        for i in range(num_candidates):
            ...
            futures.append(self.executor.submit(
                ...
                system_prompt, user_prompt, temp, top_k, top_p, num_predict
            ))
        ...
        for future in as_completed(futures):
            ...
            if candidate and candidate not in unique_candidates:
                if not self.is_semantically_similar(candidate, all_candidates):
                    unique_candidates.add(candidate)
                    all_candidates.append(candidate)

        if not all_candidates:
            return ["ARGUS encountered a communication error..."]
    ...
    def ga_rerank_candidates(self, user_input, candidates,
        ...
        If the longest candidate exceeds ga_token_threshold estimated
        tokens, skip the GA evolution — just score the raw candidates
        ...
        #fixed reference for consistent fitness
        original_candidates = list(candidates)

        #Token-length gate: skip GA on long outputs
        max_candidate_tokens = max(len(c) / 4 for c in candidates)
        skip_ga = max_candidate_tokens > ga_token_threshold

        if skip_ga:
            log_debug(f"[GA] Skipping evolution — longest candidate ~{int(max_candidate_tokens)} tokens (threshold {ga_token_threshold})")
            scores = np.array([
                self.calculate_confidencevalue_response(user_input, c, original_candidates)
                for c in candidates
            ])
            confidences = list(zip(candidates, scores))
            ...
            log_debug(f"[GA Skip] Best (confidence = {best_confidence:.2f}): {best_candidate[:80]}")
            return best_candidate, best_confidence, confidences

        #Normal GA path (short outputs)
        population = candidates[:pop_size]

        log_debug("population:", population)
        ...
        def fitness_func(candidate_text):
            ...
            #use original_candidates, not population
            return self.calculate_confidencevalue_response(user_input, candidate_text, original_candidates)
        ...
        log_debug(f"[GA Final] Best candidate (confidence = {best_confidence:.2f}): {best_candidate}")
