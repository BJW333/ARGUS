...
openai.api_key = 'sk-proj-XV8PU1y0CYJ0K9Y2DRj3T3BlbkFJPbXkagBRPmwOvinIf1Or'
...
def train_with_feedback(chatbot, original_dataset, feedback_data, epochs): #could include sampled_dataset as a variable so that the main dataset is also in the feedback data
    #combine the original dataset feedback data and conversation history
    combined_dataset = original_dataset + feedback_data #could include sampled_dataset so that the main dataset is also in the feedback data you would replace orginal dataset with that variable
...
class DynamicRewardSystem:
    ...
    def check_response_relevance(self, user_input, bot_response):
        messages = [
            ...
            Based on this interaction and what the user inputed, evaluate the chatbot's response according to the following criteria:
            ...
            Please provide a simple "yes" if the response meets all three criteria, or "no" if it fails to meet any of the criteria.
            """}
        ]
        ...
        try:
            ...
            return exit()
        #print(responsechatgpt) was used for debugging
        answer = responsechatgpt.choices[0].message['content'].strip().lower()
        
        #these two lines below are new the one right below is meant to remove all puncauation so there is no errors
        ...
        if answer == "yes":
            print("answer = yes")
            return True
        elif answer == "no": # or "was not detailed enough" in answer.lower():
            print("answer = no")
            return False
        else:       
    ...
    def evaluate_response(self, user_input, bot_response):
        ...
        if is_response_relevant == "error":
            print("An error occurred in relevance checking.")
            return 0
        ...
        #determine if the bot's sentiment aligns with the user's sentiment
            ...
            self.reward_score -= 1            
            print("The statement is not relevant, penalizing")            
            return -1
        else:
            print('error')
            return 0
        ...
        #         print('The statement is relevent and makes sense')
        #         return 1    
        #this is to give negative rewards
        # elif is_response_relevant == False: 
...
def generalvoiceinput(): 
    ...
    with sr.Microphone() as source:
        ...
        print("Listening...")  
        try: 
            voiceinputauido = recognizer.listen(source)
...
def process_user_input(user_input):
    ...
    #usage of training with feedback and historical data
        ...
        #these 4 lines below are a test of how it responds in relation to the intial response from the ai and giving it a reward wether negative or postive to learn from instead of a fake reward after correction
        # reward = reward_system.evaluate_response(user_input, response)
        # print("These stats are the reward for the inintal uncorrected response")
